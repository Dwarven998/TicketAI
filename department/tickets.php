...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_ticket') {
    ...
    // Generate collision-safe ticket number using uniqid (no rand collision risk)
    do {
        $ticket_number = 'TK' . date('y') . strtoupper(substr(uniqid(), -6));
        $chk = $pdo->prepare("SELECT id FROM tickets WHERE ticket_number = ?");
        $chk->execute([$ticket_number]);
    } while ($chk->fetch());

    try {
        $pdo->beginTransaction();

        // ── WORKAROUND: tickets.id has no AUTO_INCREMENT ─────────────────────
        // Manually calculate the next safe ID (same pattern as client_submit.php)
        $stmtMax        = $pdo->query("SELECT MAX(id) FROM tickets");
        $next_ticket_id = intval($stmtMax->fetchColumn()) + 1;

        $stmt = $pdo->prepare("
            INSERT INTO tickets
                (id, ticket_number, title, description, priority, requester_id,
                 assigned_to, category_id, status, created_at, updated_at)
        ...
        $pdo->prepare("
            ...
            VALUES (?, 'pending', 'pending', ?, 'Ticket created and assigned by campus admin', NOW())
        ")->execute([$next_ticket_id, $user_id]);
...
if ($q !== '') {
    if (preg_match('/^\d{4}$/', $q)) {
        ...
        $where_conditions[] = "(t.title LIKE ? OR t.description LIKE ? OR t.ticket_number LIKE ? OR CONCAT(u.first_name,' ',u.last_name) LIKE ? OR t.client_name LIKE ? OR u.user_type LIKE ? OR t.priority LIKE ? OR t.status LIKE ? OR c.name LIKE ?)";
        $s = "%$q%";
        for ($i = 0; $i < 9; $i++) $params[] = $s;
    }
}
...
try {
    $stmt = $pdo->prepare("
        SELECT t.*,
               ...
               CASE WHEN t.is_client=1 THEN t.client_email ELSE u.email END as requester_email,
               CASE WHEN t.is_client=1 THEN CONCAT('Client (',t.client_department,')') ELSE u.user_type END as requester_user_type,
        ...
        <main class="dashboard-content">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-shrink-0">
                <h1 class="h4 fw-bold text-dark mb-0 d-flex align-items-center">
                    <i class="fas fa-ticket-alt text-success me-2"></i>My Worklogs
                </h1>
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-sm btn-primary rounded-3 shadow-sm border fw-medium px-3"
            ...
            <?php if (isset($_GET['success'])): ?>
                <div class="alert alert-success alert-dismissible fade show border-0 shadow-sm rounded-3 py-2 flex-shrink-0">
                    <i class="fas fa-check-circle me-2"></i> Ticket created and assigned successfully.
                    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                </div>
            <?php endif; ?>
            <?php if (isset($error_msg)): ?>
                <div class="alert alert-danger alert-dismissible fade show border-0 shadow-sm rounded-3 py-2 flex-shrink-0">
                    ...
                    <form method="GET">
                        <div class="input-group input-group-sm mb-1">
                            ...
                            <input type="text" class="form-control border-start-0 ps-0" name="q"
                                ...
                                placeholder="Search by ticket #, requester, status, date...">
                        ...
                        <small class="text-muted d-block" style="font-size:0.75rem;">Tip: Search by ticket #, subject, campus, requester, priority, status, or date (YYYY, YYYY-MM, YYYY-MM-DD)</small>
                    </form>
                </div>
            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="fw-medium text-dark" style="font-size:0.9rem;">
                                                    <?php echo htmlspecialchars(substr($ticket['title'], 0, 45)); ?><?php echo strlen($ticket['title']) > 45 ? '...' : ''; ?>
                                                </div>
                                            ...
                                            <td><span class="text-dark fw-medium" style="font-size:0.85rem;"><?php echo htmlspecialchars($ticket['requester_user_type'] ?? 'N/A'); ?></span></td>
                                            <td>
                                            ...
                                            <td class="text-end pe-4">
                                                <a href="view.php?id=<?php echo $ticket['id']; ?>"
                                                    class="btn btn-sm btn-light text-success border shadow-sm rounded-circle"
                                                    style="width:32px;height:32px;padding:0;line-height:30px;" title="Review Ticket">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light border-bottom-0">
                    <h5 class="modal-title fw-bold text-dark">Create &amp; Assign Ticket</h5>

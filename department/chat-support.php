...
try {
    $stmt = $pdo->prepare("
        ...
        WHERE t.department_id = ? AND t.status IN ('pending','in_progress','unresolved','new','assigned','reopen','on_hold')
                        ...
                        <div class="card border-0 shadow-sm">
                            ...
                            <div class="card-body">
                                ...
                                try {
                                    ...
                                    $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM tickets WHERE department_id = ? AND status IN ('resolved', 'closed')");
                                    $stmt->execute([$department_id]);
                                    $total_resolved = $stmt->fetch()['total'];

                                    $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM tickets WHERE department_id = ? AND status IN ('pending', 'in_progress', 'unresolved', 'new', 'assigned', 'reopen', 'on_hold')");
                                    $stmt->execute([$department_id]);
